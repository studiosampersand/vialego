...
function reportImpactMetrics(trips){
  ...
  const referenceCo2=(cyclingKm+publicTransportKm+electricCarKm)*REPORT_ASSUMPTIONS.carCo2KgPerKm;
  const actualCo2=publicTransportKm*REPORT_ASSUMPTIONS.publicTransportCo2KgPerKm+(electricCarKm*REPORT_ASSUMPTIONS.evKwhPer100Km/100+ebikeKm*REPORT_ASSUMPTIONS.ebikeKwhPer100Km/100)*REPORT_ASSUMPTIONS.electricityCo2KgPerKwh;
  const estimatedCo2Saved=Math.max(0,referenceCo2-actualCo2);
  const comparedKm=cyclingKm+electricCarKm;
  const avoidedPetrolLitres=comparedKm*REPORT_ASSUMPTIONS.petrolLitresPer100Km/100;
  const cyclingAvoidedPetrolLitres=cyclingKm*REPORT_ASSUMPTIONS.petrolLitresPer100Km/100;
  const cyclingElectricityCost=ebikeKm*REPORT_ASSUMPTIONS.ebikeKwhPer100Km/100*.35;
  const cyclingCostSaved=Math.max(0,cyclingAvoidedPetrolLitres*REPORT_ASSUMPTIONS.petrolPricePerLitre-cyclingElectricityCost);
  const electricityCost=(electricCarKm*REPORT_ASSUMPTIONS.evKwhPer100Km/100+ebikeKm*REPORT_ASSUMPTIONS.ebikeKwhPer100Km/100)*.35;
  const estimatedCostSaved=Math.max(0,avoidedPetrolLitres*REPORT_ASSUMPTIONS.petrolPricePerLitre-electricityCost);
  return {bicycleKm,ebikeKm,cyclingKm,publicTransportKm,electricCarKm,estimatedCo2Saved,avoidedPetrolLitres,cyclingAvoidedPetrolLitres,cyclingCostSaved,estimatedCostSaved};
}
...
function renderStats(){
  ...
  statsContent.innerHTML=`
    <div class="report-filter card"><div class="section-head"><div><h3>${t('Period')}</h3><p class="muted">${escapeHtml(bounds.label)}${selectedVehicle?` - ${escapeHtml(selectedVehicle.name)}`:''}</p></div></div><div class="report-filter-chips"><button data-report-period="month" class="${reportPeriod==='month'?'active':''}">${t('Last month')}</button><button data-report-period="year" class="${reportPeriod==='year'?'active':''}">${t('This year')}</button><button data-report-period="all" class="${reportPeriod==='all'?'active':''}">${t('All')}</button><button data-report-period="custom" class="${reportPeriod==='custom'?'active':''}">${t('From–to')}</button></div><div class="report-custom-range ${customOpen}"><label>${t('From')}<input id="reportFrom" type="date" value="${safeAttr(reportCustomFrom)}"></label><label>${t('To')}<input id="reportTo" type="date" value="${safeAttr(reportCustomTo)}"></label></div><label class="report-vehicle-filter">${t('Vehicle')}<select id="reportVehicleFilter"><option value="all">${t('All vehicles')}</option>${availableVehicles.map(v=>`<option value="${safeAttr(v.id)}" ${v.id===reportVehicleId?'selected':''}>${escapeHtml(v.name)}</option>`).join('')}</select></label></div>
    ...
    <div class="card impact-card"><h3>${t('Estimated impact')}</h3>${impact.cyclingKm?`<div class="impact-row"><span>${t('Cycling distance')}</span><strong>${unitDistance(impact.cyclingKm)}</strong></div>`:''}${impact.publicTransportKm?`<div class="impact-row"><span>${t('Public transport distance')}</span><strong>${unitDistance(impact.publicTransportKm)}</strong></div>`:''}<div class="impact-row"><span>${t('Estimated CO2 avoided')}</span><strong>${impact.estimatedCo2Saved.toFixed(1)} kg</strong></div>${impact.cyclingKm?`<div class="impact-row"><span>${t('Estimated petrol avoided')}</span><strong>${impact.avoidedPetrolLitres.toFixed(1)} L</strong></div><div class="impact-row"><span>${t('Estimated costs saved')} (${t('Cycling distance')})</span><strong>${money(impact.cyclingCostSaved)}</strong></div>`:''}<p class="muted">${t('Impact assumptions')}: ${REPORT_ASSUMPTIONS.petrolLitresPer100Km.toFixed(1)} L/100 km - €${REPORT_ASSUMPTIONS.petrolPricePerLitre.toFixed(2)}/L - ${REPORT_ASSUMPTIONS.carCo2KgPerKm.toFixed(2)} kg CO2/km.</p></div>`;
  bindReportControls();
}
...
function reportFileName(){const b=reportDateBounds(),clean=x=>String(x).replace(/[^0-9A-Za-z_-]+/g,'-').replace(/^-+|-+$/g,''),vehicle=reportVehicleId==='all'?'all-vehicles':clean(vehicleById(reportVehicleId)?.name||'vehicle');return `MoBud-report-${vehicle}-${clean(b.from)}-to-${clean(b.to)}.pdf`}
async function createReportPdfBlob(){
  ...
  if(y<170){page=addPage();y=770}text(page,t('Estimated impact'),48,y,14,bold);y-=24;[[t('Cycling distance'),unitDistance(impact.cyclingKm)],[t('Public transport distance'),unitDistance(impact.publicTransportKm)],[t('Estimated CO2 avoided'),`${impact.estimatedCo2Saved.toFixed(1)} kg`],[t('Estimated petrol avoided'),`${impact.avoidedPetrolLitres.toFixed(1)} L`],[`${t('Estimated costs saved')} (${t('Cycling distance')})`,money(impact.cyclingCostSaved)]].forEach(row=>{text(page,row[0],48,y,9,font,rgb(.35,.35,.38));text(page,row[1],330,y,10,bold);y-=20});text(page,`${t('Impact assumptions')}: ${REPORT_ASSUMPTIONS.petrolLitresPer100Km.toFixed(1)} L/100 km - €${REPORT_ASSUMPTIONS.petrolPricePerLitre.toFixed(2)}/L - ${REPORT_ASSUMPTIONS.carCo2KgPerKm.toFixed(2)} kg CO2/km.`,48,y-4,7,font,rgb(.45,.45,.48));
  for(const expense of attachments){const a=safeAttachment(expense.attachment);if(!a)continue;try{if(a.type==='application/pdf'){const src=await PDFDocument.load(dataUrlBytes(a.data),{ignoreEncryption:true});const copied=await pdf.copyPages(src,src.getPageIndices());copied.forEach(p=>pdf.addPage(p))}else{const p=addPage();text(p,`${t('Receipt')} - ${expense.date||''} - ${vehicleById(expense.vehicleId)?.name||t('No vehicle')} - ${money(expense.amount||0)}`,36,805,10,bold);const img=await imageAttachmentForPdf(pdf,a);const maxW=523,maxH=720,scale=Math.min(maxW/img.width,maxH/img.height,1),w=img.width*scale,h=img.height*scale;p.drawImage(img,{x:(PAGE_W-w)/2,y:50+(720-h)/2,width:w,height:h})}}catch(err){const p=addPage();text(p,`${t('Receipt could not be embedded')}: ${a.name||t('Unnamed file')}`,48,790,11,bold)}}
...
async function exportPdfReport(){
  ...
  try{
    ...
    if(mobile){
      ...
      setTimeout(()=>URL.revokeObjectURL(url),300000);toast(t('PDF ready.'));
    }else{
      const a=document.createElement('a');a.href=url;a.download=name;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),15000);toast(t('PDF exported.'));
    }
...
async function searchTripOther(which,text){
  ...
  try{
    const r=await fetch(`${API}/geocode?text=${encodeURIComponent(text)}&country=${encodeURIComponent(state.settings.country||'')}`,{cache:'no-store'});if(!r.ok)throw new Error(r.status);

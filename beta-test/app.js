...
function renderStats(){
  ...
  statsContent.innerHTML=`
    ...
    <div class="card impact-card"><h3>${t('Estimated impact')}</h3><div class="impact-row"><span>${t('Cycling distance')}</span><strong>${unitDistance(cyclingKm)}</strong></div><div class="impact-row"><span>${t('Estimated CO₂ avoided')}</span><strong>${estimatedCo2Saved.toFixed(1)} kg</strong></div><p class="muted">${t('Estimated against an avoided average car journey. Actual savings vary by vehicle, energy source and route.')}</p></div>`;
  bindReportControls();
}
...
async function createReportPdfBlob(){
  ...
  let page=pdf.addPage([595.28,841.89]),y=790;const draw=(text,size=10,x=48,f=font)=>{page.drawText(String(text),{x,y,size,font:f,color:rgb(0.08,0.08,0.09)});y-=size+7};
  ...
  for(const v of state.vehicles){const vt=trips.filter(x=>x.vehicleId===v.id),ve=expenses.filter(x=>x.vehicleId===v.id),km=vt.reduce((a,x)=>a+Number(x.km||0),0),cost=ve.reduce((a,x)=>a+Number(x.amount||0),0);if(!vt.length&&!ve.length)continue;if(y<80){page=pdf.addPage([595.28,841.89]);y=790;page.drawText('Summary continued',{x:48,y,size:14,font:bold});y-=24}draw(`${v.name}: ${vt.length} trips · ${unitDistance(km)} · ${money(cost)}`,10)}
  for(const expense of attachments){const a=safeAttachment(expense.attachment);if(!a)continue;try{
    if(a.type==='application/pdf'){
      ...
      const img=await imageAttachmentForPdf(pdf,a);const maxW=523,maxH=745,scale=Math.min(maxW/img.width,maxH/img.height,1),w=img.width*scale,h=img.height*scale;p.drawImage(img,{x:(595.28-w)/2,y:35+(745-h)/2,width:w,height:h});
    }
...
async function printFilteredReport(){let url='';try{toast('Preparing print…');const blob=await createReportPdfBlob(),name=reportFileName();await archiveReportToDrive(blob,name);url=URL.createObjectURL(blob);const frame=document.createElement('iframe');frame.className='print-pdf-frame';frame.title=name;frame.src=url;document.body.append(frame);const cleanup=()=>{frame.remove();if(url)URL.revokeObjectURL(url)};frame.onload=()=>{setTimeout(()=>{try{const win=frame.contentWindow;win.addEventListener?.('afterprint',cleanup,{once:true});win.focus();win.print();setTimeout(()=>{if(document.body.contains(frame))cleanup()},120000)}catch(error){console.warn('Embedded print failed',error);cleanup();const opened=window.open(url,'_blank');if(!opened)toast('Open the PDF download and print it from your PDF viewer.')}},350)}}catch(e){console.error(e);if(url)URL.revokeObjectURL(url);toast('Could not prepare the print report.')}}

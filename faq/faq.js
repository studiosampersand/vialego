(function(){
  ...
  function lang(){
    ...
    return (new URLSearchParams(location.search).get('lang')||navigator.language||'nl').slice(0,2);
  }
  const locale=lang();
  fetch('../content/faq.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error();return r.json()}).then(items=>{
    host.innerHTML=items.map((item,i)=>{
      ...
      return `<article class="faq-item"><button class="faq-question" aria-expanded="false" aria-controls="answer-${i}"><span>${escapeHtml(q)}</span><span class="faq-chevron" aria-hidden="true"></span></button><div class="faq-answer hidden" id="answer-${i}">${a}</div></article>`;
    }).join('');
